...
class HybridVectorManager:
    ...
    async def upsert_docs(
        self, texts: List[str], parent_texts: List[str], user_id: str, resource_id: str
    ):
        """写入数据：同步生成双路向量"""
    ...
    async def hybrid_search(
        self, query: str, user_id: str, resource_ids: List[str], top_k: int = 5
    ) -> List[Dict]:
        """执行混合检索"""
        ...
        filter_expr = (
            f'user_id == "{user_id}" and resource_id in [{", ".join(safe_ids)}]'
        )
