...
class Paths(Base):
    ...
    pid: Mapped[str] = mapped_column(
        CHAR(36, charset="ascii", collation="ascii_general_ci"),
        primary_key=True,
...
class Users(Base):
    __tablename__ = "users"
    __table_args__ = (Index("uk_users_uname", "uname", unique=True),)
    ...
    password: Mapped[str] = mapped_column(
        String(16), nullable=False, server_default="123456", comment="User Password"
    )
    ...
    resources: Mapped[list["Resources"]] = relationship(
        "Resources", back_populates="users"
    )
    ...
    exercises: Mapped[list["Exercises"]] = relationship(
        "Exercises", back_populates="users"
    )
    flashcards: Mapped[list["Flashcards"]] = relationship(
        "Flashcards", back_populates="users"
    )
...
class Resources(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["uid"], ["users.uid"], ondelete="CASCADE", name="fk_resources_uid"
        ),
    ...
    url: Mapped[Optional[str]] = mapped_column(
        CHAR(256), comment="Resource URL (legacy or external)"
    )
    ...
    rname: Mapped[str] = mapped_column(
        String(100), nullable=False, comment="Resource name"
    )
    rtype: Mapped[str] = mapped_column(
        String(20), nullable=False, comment="Resource type (doc/video/audio/etc.)"
    )
    created_at: Mapped[datetime.datetime] = mapped_column(
        DateTime,
        nullable=False,
    ...
    content: Mapped[Optional[str]] = mapped_column(
        LONGTEXT, comment="Raw extracted text content"
    )
    summary: Mapped[Optional[str]] = mapped_column(
        Text, comment="Generated summary (optional)"
    )
    keywords: Mapped[Optional[dict]] = mapped_column(
        JSON, comment="Keywords (JSON array/object)"
    )
    status: Mapped[Optional[int]] = mapped_column(
        Integer, server_default=text("'0'"), comment="0=pending, 1=parsing, 2=parsed"
    )

    users: Mapped["Users"] = relationship("Users", back_populates="resources")
    session_resources: Mapped[list["SessionResources"]] = relationship(
        "SessionResources", back_populates="resources"
    )
...
class Sessions(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["uid"], ["users.uid"], ondelete="CASCADE", name="fk_sessions_uid"
        ),
    ...
    session_resources: Mapped[list["SessionResources"]] = relationship(
        "SessionResources", back_populates="sessions"
    )
...
class SessionResources(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["resources.rid"],
            ondelete="CASCADE",
            name="fk_session_resources_rid",
        ),
        ForeignKeyConstraint(
            ["sid"],
            ["sessions.sid"],
            ondelete="CASCADE",
            name="fk_session_resources_sid",
        ),
    ...
    resources: Mapped["Resources"] = relationship(
        "Resources", back_populates="session_resources"
    )
    sessions: Mapped["Sessions"] = relationship(
        "Sessions", back_populates="session_resources"
    )
...
class Units(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["pid"], ["paths.pid"], ondelete="CASCADE", name="fk_units_pid"
        ),
        ForeignKeyConstraint(
            ["sid"], ["sessions.sid"], ondelete="SET NULL", name="fk_units_sid"
        ),
        ForeignKeyConstraint(
            ["uid"], ["users.uid"], ondelete="CASCADE", name="fk_units_uid"
        ),
    ...
    goal: Mapped[str] = mapped_column(
        String(200), nullable=False, comment="Learning goal"
    )
    guide: Mapped[str] = mapped_column(
        Text, nullable=False, comment="Learning guide/outline"
    )
    created_at: Mapped[datetime.datetime] = mapped_column(
        DateTime,
        nullable=False,
    ...
    sid: Mapped[Optional[str]] = mapped_column(
        CHAR(36, charset="ascii", collation="ascii_general_ci"),
        comment="Optional session ID (UUID)",
    )
    core_concepts: Mapped[Optional[dict]] = mapped_column(
        JSON, comment="Core concepts (JSON array/object)"
    )
    completed_at: Mapped[Optional[datetime.datetime]] = mapped_column(
        DateTime, comment="Completed time (optional)"
    )
    status: Mapped[Optional[int]] = mapped_column(
        Integer,
    ...
    sessions: Mapped[Optional["Sessions"]] = relationship(
        "Sessions", back_populates="units"
    )
    users: Mapped["Users"] = relationship("Users", back_populates="units")
    exercises: Mapped[list["Exercises"]] = relationship(
        "Exercises", back_populates="unit"
    )
    flashcards: Mapped[list["Flashcards"]] = relationship(
        "Flashcards", back_populates="unit"
    )
...
class Exercises(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["uid"], ["users.uid"], ondelete="CASCADE", name="fk_exercises_uid"
        ),
        ForeignKeyConstraint(
            ["unit_id"],
            ["units.unit_id"],
            ondelete="CASCADE",
            name="fk_exercises_unit_id",
        ),
    ...
    correct_answer: Mapped[str] = mapped_column(
        Text, nullable=False, comment="Correct answer"
    )
    created_at: Mapped[datetime.datetime] = mapped_column(
        DateTime,
        nullable=False,
    ...
    options: Mapped[Optional[dict]] = mapped_column(
        JSON, comment="Options (JSON array/object, optional)"
    )
    explanation: Mapped[Optional[str]] = mapped_column(
        Text, comment="Explanation (optional)"
    )
...
class Flashcards(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["uid"], ["users.uid"], ondelete="CASCADE", name="fk_flashcards_uid"
        ),
        ForeignKeyConstraint(
            ["unit_id"],
            ["units.unit_id"],
            ondelete="CASCADE",
            name="fk_flashcards_unit_id",
        ),
        Index("idx_flashcards_uid_created", "uid", "created_at"),
        Index("idx_flashcards_unit_id", "unit_id"),
    )

    fcid: Mapped[str] = mapped_column(
        CHAR(36, charset="ascii", collation="ascii_general_ci"),
        primary_key=True,
    ...
    question: Mapped[str] = mapped_column(
        Text, nullable=False, comment="Front side / question"
    )
    answer: Mapped[str] = mapped_column(
        Text, nullable=False, comment="Back side / answer"
    )
    ...
    last_reviewed_at: Mapped[Optional[datetime.datetime]] = mapped_column(
        DateTime, comment="Last reviewed time (optional)"
    )
